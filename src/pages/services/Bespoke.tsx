import { Link } from "react-router-dom";
import { useTranslation } from "react-i18next";
import { ArrowLeft, CheckCircle } from "lucide-react";
import Button from "../../components/Button";
import Seo from "../../components/Seo";
import bespokeImage from "../../imgs/cats.jpeg";

const GOOD_FOR_IDS = ["indoor", "unusual", "custom"];
const HOW_IDS = ["enquire", "quote", "book"];

const Bespoke = () => {
  const { t } = useTranslation(["bespoke", "seo"]);

  return (
    <section className="max-w-5xl mx-auto px-6 md:px-12 py-14">
      <Seo
        title={t("seo:bespoke.title")}
        description={t("seo:bespoke.description")}
        path="/services/bespoke-sessions"
      />
      <Link
        to="/#services"
        className="inline-flex items-center gap-1 text-xs font-semibold tracking-widest text-gray-500 hover:text-gray-900 transition-colors uppercase"
      >
        <ArrowLeft size="1em" />
        {t("backToServices")}
      </Link>

      <div className="mt-8 grid lg:grid-cols-2 gap-12 items-center">
        <div className="rounded-2xl overflow-hidden h-[420px]">
          <img
            src={bespokeImage}
            alt={t("imageAlt")}
            className="w-full h-full object-cover"
          />
        </div>

        <div>
          <p className="text-xs font-semibold tracking-widest text-gray-500 mb-2 uppercase">
            {t("eyebrow")}
          </p>
          <h1 className="font-serif text-4xl md:text-5xl text-gray-900">
            {t("title")}
          </h1>
          <p className="mt-6 text-gray-600 leading-relaxed max-w-md">
            {t("intro")}
          </p>
          <div className="mt-8">
            <Button href="/contact">{t("getQuote")}</Button>
          </div>
        </div>
      </div>

      <div className="mt-20 grid md:grid-cols-2 gap-12">
        <div>
          <h2 className="font-serif text-2xl text-gray-900 mb-6">
            {t("goodForHeading")}
          </h2>
          <ul className="space-y-4">
            {GOOD_FOR_IDS.map((id) => (
              <li key={id} className="flex items-start gap-3 text-gray-700">
                <CheckCircle
                  className="text-primary mt-0.5 flex-shrink-0"
                  size={20}
                />
                <span>{t(`goodFor.${id}`)}</span>
              </li>
            ))}
          </ul>
        </div>

        <div className="bg-white border border-gray-200 rounded-2xl p-8">
          <p className="text-xs font-semibold tracking-widest text-gray-500 mb-2 uppercase">
            {t("howHeading")}
          </p>
          <ol className="mt-4 space-y-4">
            {HOW_IDS.map((id, index) => (
              <li key={id} className="flex items-start gap-3 text-sm text-gray-700">
                <span className="flex-shrink-0 w-6 h-6 rounded-full bg-primary/10 text-primary text-xs font-semibold flex items-center justify-center">
                  {index + 1}
                </span>
                <span>{t(`how.${id}`)}</span>
              </li>
            ))}
          </ol>

          <div className="mt-6">
            <Button href="/contact" className="w-full text-center">
              {t("getQuote")}
            </Button>
          </div>
        </div>
      </div>
    </section>
  );
};

export default Bespoke;
